...
quickFilters.Assistant = {
  ...
  loadAssistant: async function () {
    ...
    dlgButtons["extra1"].addEventListener("click", (_e) => {
      return quickFilters.Assistant.next();
    });
    dlgButtons["cancel"].addEventListener("click", (_e) => {
    ...
    if (quickFilters.Util.licenseInfo) {
      if (quickFilters.Util.licenseInfo.isValid) {
        ...
        dlgButtons["extra2"].label = quickFilters.Util.getBundleString(
          "quickfilters.notification.premium.btn.renewLicense"
        );
        dlgButtons["extra2"].addEventListener("click", (_e) => {
          quickFilters.Util.showLicenseDialog("assistant_renew");
        });
      } else {
        // old case (donate)
        dlgButtons["extra2"].addEventListener("click", (_e) => {
          quickFilters.Util.showLicensePage();
        });
      }
    }
      ...
      if (filterCount) {
        for (let i = 0; i < filterCount; i++) {
          ...
          if (token[0] && token[0].indexOf("quickFilterCustomTemplate") == 0) {
            ...
            if (token[1]) {
              let listItem = document.createXULElement
                  ? document.createXULElement("richlistitem")
                  : document.createElement("richlistitem"),
                description = document.createXULElement
    ...
    if (matchingFilters.length > 0) {
      ...
      switch (params.cmd) {
        case "mergeList":
          ...
          document.getElementById("mergeSummary").innerText = util.getBundleString(
            "qf.description.mergeAddSummary"
          );
          document.getElementById("mergeInstructions").innerText = util.getBundleString(
            "qf.description.mergeAddInstructions"
          );
          document.getElementById("chkMerge").innerText = util.getBundleString(
        ...
        default:
          ...
          document.getElementById("filterDescription").innerText = util.getBundleString(
    ...
    try {
      if (params.cmd == "new") {
        if (params.preview) {
          ...
          document.getElementById("previewFrom").value =
            document.getElementById("previewFrom").value + " " + preview.author;
          document.getElementById("previewTo").value =
            document.getElementById("previewTo").value + " " + preview.recipients;
          ...
          document.getElementById("previewDate").value =
            document.getElementById("previewDate").value + " " + preview.date;
          document.getElementById("previewLines").value =
            document.getElementById("previewLines").value + " " + preview.lines;
          document.getElementById("previewCaption").label = "{0} Email(s)".replace(
            "{0}",
            preview.msgCount
          );
        }
    ...
    if (isMergePossible) {
      ...
      if (prefs.getBoolPref("merge.autoSelect") || prefs.getBoolPref("merge.silent")) {
